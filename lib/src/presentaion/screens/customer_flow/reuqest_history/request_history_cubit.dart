...
class RequestHistoryCubit extends Cubit<RequestHistoryState> {
  ...
  RequestHistoryCubit(this._jobManagementService) : super(RequestHistoryInitial());
  ...
  Future<void> fetchWorkOrders() async {
    ...
    final completedResult = await _jobManagementService.getCompletedWorkOrdersForCustomer(1);
    final cancelledResult = await _jobManagementService.getCanceledWorkOrdersForCustomer(1); 
    final newRequestResult = await _jobManagementService.getCustomerNewRequests(1); 

    if (completedResult.isLeft() || cancelledResult.isLeft() || newRequestResult.isLeft()){
      final failure = completedResult.fold((l) => l, (r) => null) ?? cancelledResult.fold((l) => l, (r) => null) ?? newRequestResult.fold((l) => l, (r) => null);
