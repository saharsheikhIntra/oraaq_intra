...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  if(data['screen']=='offer-recieved'){
    CustomerNewRequestDto recievedReq = CustomerNewRequestDto.fromMap(data['offer']);
      ValueNotifier<CustomerNewRequestDto> currentRequest =
                                            ValueNotifier(recievedReq);
      navigatorKey.currentState?.pushNamed(RouteConstants.offeredReceivedScreenRoute,arguments: OfferRecievedArguments(currentRequest));
  }else{
    log(data['screen']);
    navigatorKey.currentState?.pushNamed(RouteConstants.merchantViewAllOrdersRoute);
  }
}
...
class FirebaseAPI{
  ...
  void handleMessage(RemoteMessage? message) {
    ...
    if(notificationData['screen']=='offer-recieved'){
      ...
      CustomerNewRequestDto recievedReq = CustomerNewRequestDto.fromMap(notificationData['offer']);
      ValueNotifier<CustomerNewRequestDto> currentRequest =
                                            ValueNotifier(recievedReq);
      navigatorKey.currentState?.pushNamed(RouteConstants.offeredReceivedScreenRoute,arguments: OfferRecievedArguments(currentRequest));
  }else{
  ...
  Future initLocalNotifications()async{
    ...
    await _localNotifications.initialize(settings,onDidReceiveNotificationResponse: (details) {
